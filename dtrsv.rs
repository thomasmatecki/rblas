...
/* Subroutine */
#[no_mangle]
pub unsafe extern "C" fn f2c_dtrsv(mut uplo: *mut libc::c_char,
...
/*     .. */
    ...
    if info != 0 as libc::c_int as libc::c_long {
        xerbla__0(b"DTRSV \x00" as *const u8 as *const libc::c_char as
                ...
                while j >= 1 as libc::c_int as libc::c_long {
                    if *x.offset(j as isize) != 0.0f64 {
                        if nounit != 0 {
                            ...
                            *fresh0 /= *a.offset((j + j * a_dim1) as isize)
                        }
                ...
                while j >= 1 as libc::c_int as libc::c_long {
                    if *x.offset(jx as isize) != 0.0f64 {
                        if nounit != 0 {
                            ...
                            *fresh2 /= *a.offset((j + j * a_dim1) as isize)
                        }
            ...
            while j <= i__1 {
                if *x.offset(j as isize) != 0.0f64 {
                    if nounit != 0 {
                        ...
                        *fresh4 /= *a.offset((j + j * a_dim1) as isize)
                    }
                    temp = *x.offset(j as isize);
                    i__2 = *n;
                    i__ = j + 1 as libc::c_int as libc::c_long;
            ...
            while j <= i__1 {
                if *x.offset(jx as isize) != 0.0f64 {
                    if nounit != 0 {
                        ...
                        *fresh6 /= *a.offset((j + j * a_dim1) as isize)
                    }
                    temp = *x.offset(jx as isize);
                    ix = jx;
                    i__2 = *n;
