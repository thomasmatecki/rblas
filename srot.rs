...
/* Subroutine */
#[no_mangle]
pub unsafe extern "C" fn f2c_srot(mut n: *mut integer, mut sx: *mut real,
...
/*     .. */
    ...
    if *incx == 1 as libc::c_int as libc::c_long &&
        ...
        while i__ <= i__1 {
            stemp =
                *c__ * *sx.offset(i__ as isize) +
                    *s * *sy.offset(i__ as isize);
            *sy.offset(i__ as isize) =
                *c__ * *sy.offset(i__ as isize) -
                    *s * *sx.offset(i__ as isize);
...
/*         to 1 */
        ...
        while i__ <= i__1 {
            stemp =
                *c__ * *sx.offset(ix as isize) + *s * *sy.offset(iy as isize);
            *sy.offset(iy as isize) =
                *c__ * *sy.offset(iy as isize) - *s * *sx.offset(ix as isize);
