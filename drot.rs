...
pub unsafe extern "C" fn f2c_drot(
    ...
    if *incx == 1 as libc::c_int as libc::c_long && *incy == 1 as libc::c_int as libc::c_long {
        ...
        while i__ <= i__1 {
            dtemp = *c__ * *dx.offset(i__ as isize) + *s * *dy.offset(i__ as isize);
            *dy.offset(i__ as isize) =
                *c__ * *dy.offset(i__ as isize) - *s * *dx.offset(i__ as isize);
        ...
        i__ = 1 as libc::c_int as integer;
        while i__ <= i__1 {
            dtemp = *c__ * *dx.offset(ix as isize) + *s * *dy.offset(iy as isize);
            *dy.offset(iy as isize) = *c__ * *dy.offset(iy as isize) - *s * *dx.offset(ix as isize);
