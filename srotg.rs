...
/* Subroutine */
#[no_mangle]
pub unsafe extern "C" fn f2c_srotg(
    mut sa: *mut real,
    mut sb: *mut real,
    mut c__: *mut real,
    mut s: *mut real,
    ...
    /*     .. */
    roe = *sb;
    if (if *sa >= 0 as libc::c_int as libc::c_float {
        *sa
    } else {
        -*sa
    }) as doublereal
    ...
    scale = ((if *sa >= 0 as libc::c_int as libc::c_float {
        *sa
    } else {
        -*sa
    }) as doublereal
    ...
    if scale != 0.0f32 {
        ...
        r__2 = *sb / scale;
        r__ =
            (scale as libc::c_double * sqrt((r__1 * r__1 + r__2 * r__2) as libc::c_double)) as real;
        ...
        *s = *sb / r__;
        ...
        if (if *sb >= 0 as libc::c_int as libc::c_float {
            *sb
        } else {
            -*sb
        }) as doublereal
